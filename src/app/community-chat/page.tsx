...
import { Send, Users, MessageSquare, Loader2, ShieldAlert, Terminal } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen h-[calc(100vh-4rem)] flex flex-col overflow-hidden bg-background">
      ...
      <main className="flex-1 container mx-auto px-4 py-4 md:py-8 max-w-4xl flex flex-col min-h-0">
        ...
        <Card className="flex-1 flex flex-col border-none shadow-2xl bg-card overflow-hidden rounded-[1.5rem] md:rounded-[2rem] min-h-0">
          <CardHeader className="bg-red-600 text-white py-3 md:py-4 px-6 flex flex-row items-center justify-between">
            <div className="flex items-center gap-3">
              <Users className="w-5 h-5 text-white" />
              <CardTitle className="text-sm md:text-lg uppercase font-black">Main Lobby</CardTitle>
            </div>
          </CardHeader>
          ...
          <CardContent className="flex-1 flex flex-col p-0 min-h-0 bg-card">
              ...
              <div className="space-y-4">
                {messagesLoading ? (
                  ...
                  <div key={msg.id || idx} className={cn("flex gap-3", msg.senderId === user?.uid ? "flex-row-reverse" : "flex-row")}>
                    ...
                    <div className={cn("max-w-[80%] space-y-1 flex flex-col", msg.senderId === user?.uid ? "items-end text-right" : "items-start")}>
                      <span className="text-[9px] font-black uppercase text-muted-foreground">{msg.senderName}</span>
                      <div className={cn("px-4 py-2 rounded-2xl text-sm shadow-sm font-bold bg-white text-black border border-border/50")}>
                        {msg.text}
                      </div>
                    </div>
                  </div>
            ...
            <div className="p-4 bg-muted/50 dark:bg-zinc-900 border-t mt-auto">
              {!user ? (
                <div className="flex items-center justify-center p-2 text-muted-foreground text-xs font-bold uppercase tracking-widest gap-2">
                ...
                <form onSubmit={handleSendMessage} className="flex gap-2">
                  <Input
                    ...
                    className="flex-1 rounded-full h-12 px-6 bg-white text-black border-zinc-200 placeholder:text-zinc-400 focus-visible:ring-accent shadow-sm"
                  />
                  <Button type="submit" disabled={!messageText.trim()} title="Send message" className="h-12 w-12 rounded-full bg-accent text-white hover:bg-accent/90 shrink-0 shadow-lg">
