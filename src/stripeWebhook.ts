...
export const stripeWebhook = onRequest(
  ...
  async (req, res) => {
    ...
    if (event.type === "checkout.session.completed") {
      ...
      // Theme purchase automation
      const themeId = session.metadata?.themeId;
      const buyerId = session.metadata?.buyerId;
      if (themeId && buyerId) {
        // Update user's store profile with new theme
        const storeSnap = await db.collection("storefronts").where("ownerUid", "==", buyerId).limit(1).get();
        if (storeSnap.empty) {
          console.error("Storefront not found for theme purchase", buyerId);
        ...
        await db.collection("users").doc(buyerId).collection("notifications").add({
          ...
          body: `Your store theme has been updated to ${themeId}.`,
          relatedId: themeId,
          read: false,
          createdAt: admin.firestore.Timestamp.now(),
        });
        res.status(200).send("Theme activated");
        return;
      }
